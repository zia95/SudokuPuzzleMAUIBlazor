...
export var generate = function (num) {
	...
	while (num > 0) {
		...
		ss.setPrintStyle(qqwing.PrintStyle.ONE_LINE);

		var havePuzzle = ss.generatePuzzleSymmetry(qqwing.Symmetry.NONE);

		if (!havePuzzle) {
			continue;
		}

		var solutions = ss.countSolutions();
		...
		if (ss.getDifficulty() === qqwing.Difficulty.UNKNOWN) {
			continue;
		}

		var puz =
		{
			puzzle: ss.getPuzzleString(),
			solution: ss.getSolutionString(),

			givenCount: ss.getGivenCount(),
			singleCount: ss.getSingleCount(),
			hiddenSingleCount: ss.getHiddenSingleCount(),
			nakedPairCount: ss.getNakedPairCount(),
			hiddenPairCount: ss.getHiddenPairCount(),
			pointingPairTripleCount: ss.getPointingPairTripleCount(),
			boxReductionCount: ss.getBoxLineReductionCount(),
			guessCount: ss.getGuessCount(),
			backtrackCount: ss.getBacktrackCount(),
			difficultyString: ss.getDifficultyAsString()
		};
